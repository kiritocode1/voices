...
function loadCfgs(onnxDir: string): TtsConfig {
	const cfgPath = path.join(onnxDir, "tts.json");
	return readJsonFile<TtsConfig>(cfgPath);
}
...
function loadTextProcessor(onnxDir: string): UnicodeProcessor {
	...
	const indexer = readJsonFile<UnicodeIndexerJson>(unicodeIndexerPath);
...
async function loadTextToSpeechInternal(onnxDir: string): Promise<{ textToSpeech: TextToSpeech; cfgs: TtsConfig }> {
	...
	const [dpOrt, textEncOrt, vectorEstOrt, vocoderOrt] = await Promise.all([
		loadOnnx(dpPath, sessionOptions),
		loadOnnx(textEncPath, sessionOptions),
		loadOnnx(vectorEstPath, sessionOptions),
		loadOnnx(vocoderPath, sessionOptions),
	]);
...
function resolveVoiceStylePath(id: VoiceStyleId): string {
	const filename = `${id}.json`;
	return path.join(process.cwd(), "model", "voice_styles", filename);
}
...
function loadVoiceStyleFromFile(stylePath: string): Style {
	const voiceStyle = readJsonFile<VoiceStyleJson>(stylePath);
...
function writeWavToBuffer(audioData: number[], sampleRate: number): Buffer {
	const numChannels = 1;
...
let cachedTextToSpeech: TextToSpeech | null = null;
let cachedConfig: TtsConfig | null = null;
const styleCache: Map<VoiceStyleId, Style> = new Map<VoiceStyleId, Style>();

async function getTextToSpeech(): Promise<{
	textToSpeech: TextToSpeech;
	cfgs: TtsConfig;
}> {
	if (cachedTextToSpeech && cachedConfig) {
		return { textToSpeech: cachedTextToSpeech, cfgs: cachedConfig };
	}

	const onnxDir = resolveModelDir();
	const { textToSpeech, cfgs } = await loadTextToSpeechInternal(onnxDir);

	cachedTextToSpeech = textToSpeech;
	cachedConfig = cfgs;

	return { textToSpeech, cfgs };
}

function getOrLoadStyle(voiceStyle: VoiceStyleId): Style {
	const existing = styleCache.get(voiceStyle);
	if (existing) {
		return existing;
	}

	const stylePath = resolveVoiceStylePath(voiceStyle);
	const style = loadVoiceStyleFromFile(stylePath);
	styleCache.set(voiceStyle, style);
	return style;
}

export async function synthesizeToWavBuffer(options: SynthesizeOptions): Promise<SynthesisResult> {
	...
	const style = getOrLoadStyle(voiceStyle);
